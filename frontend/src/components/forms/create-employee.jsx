...
export function CreateEmployeeForm({ onFormSubmit, className }) {
  ...
  async function onSubmit(values) {
    ...
    const employeeRole = ROLES.EMPLOYEE.id;
    data.append("roles", JSON.stringify([employeeRole]));
    ...
    if (res.error) {
      toast.error(res.error.message, { id: "create-employee" });
      throw res.error.message;
    }

    toast.success(res.message, { id: "create-employee" });
  ...
  return (
    <div className={cn("w-full max-w-prose space-y-4 mx-auto", className)}>
      ...
      <Title className="text-xl font-semibold">Nuevo empleado</Title>
