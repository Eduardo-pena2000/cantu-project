...
import { administrativeRoles } from "@/data";
...
const formSchema = z.object({
  ...
  roles: z.array(z.string().min(1)).min(1).nonempty("Debes seleccionar por lo menos un rol."),
});

export function CreateUserForm({ onFormSubmit, className }) {
  ...
  async function onSubmit(values) {
    ...
    for (const field in values) {
      if (field === "roles" && Object.prototype.hasOwnProperty.call(values, field)) {
        data.append("roles", JSON.stringify(values[field]));
    ...
    if (res.error) {
      toast.error(res.error.message, { id: "create-user" });
      throw res.error.message;
    }

    toast.success(res.message, { id: "create-user" });
  ...
  return (
    <div className={cn("w-full max-w-prose space-y-4 mx-auto", className)}>
      <div className="h-44 relative">
        <div className="bg-gradient-to-r from-purple-500 to-indigo-600 h-32 w-full rounded-3xl relative">
          <Image
            ...
            src={img ? URL.createObjectURL(img) : "/user-round.svg"}
      ...
      <Title className="text-xl font-semibold">Nuevo usuario</Title>
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel className="w-fit">Roles</FormLabel>
                <FormControl>
                  <MultiSelect
                    modalPopover
                ...
                <FormDescription>
                  Asigna roles al usuario para definir sus permisos en la plataforma.
                </FormDescription>
                <FormMessage />
              </FormItem>
            )}
