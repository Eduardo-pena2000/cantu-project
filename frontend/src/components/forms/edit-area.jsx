...
import { updateAreaById } from "@/actions/areas";
import { editAreaSchema, getDefaultValues } from "@/schemas/area";
...
export function EditAreaForm({ area, onFormSubmit, className }) {
  ...
  async function onSubmit(values) {
    const res = await updateAreaById(area.id, values);
    ...
    if (res.error) {
      toast.error(res.error.message, { id: "update-area" });
      throw res.error.message;
    }

    toast.success(res.message, { id: "update-area" });
            ...
            <FormItem>
              ...
              <FormControl>
                <AsyncSelect
                  ...
                  value={field.value}
                  initialInputValue={area.manager?.shortFullName}
