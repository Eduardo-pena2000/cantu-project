...
import { paginationDto, scheduleEmployeeDto } from "@/dtos";
...
export const GET = auth(async function GET(request, { params }) {
  try {
    ...
    const res = await fetch(
      `${process.env.API_URL}/user/schedule/${Number(id)}?${queries.toString()}`,
      {
        method: "GET",
        headers: {
    ...
    const scheduleEmployees = data.map((scheduleEmployee) => scheduleEmployeeDto(scheduleEmployee));
    ...
    return new NextResponse(
      JSON.stringify({
        status: res.status,
        data: { employees: scheduleEmployees, pagination },
      }),
      { status: res.status }
    );
  } catch (error) {
