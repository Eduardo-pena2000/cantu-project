...
export default async function Page({ searchParams }) {
  ...
  return (
    <div className="flex flex-col gap-6 animate-fade-in relative max-w-[100vw] overflow-x-hidden pb-10">
      ...
      <Tabs value="record-attendance" className="space-y-6 w-full relative z-10">
        <TabsList className="grid w-full max-w-md grid-cols-2 bg-background/60 p-1.5 rounded-xl shadow-sm border border-border/60 backdrop-blur-md">
          <TabsTrigger value="record-attendance" data-state="active" className="rounded-lg py-2 data-[state=active]:bg-sidebar-primary data-[state=active]:text-sidebar-primary-foreground data-[state=active]:shadow-md transition-all duration-300 font-medium">Registrar asistencia</TabsTrigger>
          <TabsTrigger asChild value="recorded" className="rounded-lg py-2 text-muted-foreground hover:bg-muted/50 hover:text-foreground transition-all duration-300 font-medium text-center">
            <Link
        ...
        <div className="bg-card/60 backdrop-blur-xl border border-border/50 shadow-sm rounded-2xl p-6 relative overflow-hidden">
          ...
          <TabsContent value="record-attendance" className="mt-0 outline-none">
