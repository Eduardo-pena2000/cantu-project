...
export function UserSchedule({ teamId, employeeId, schedules, employeeSchedule }) {
  const [open, setOpen] = React.useState(false);
  const [days, setDays] = React.useState(
    new Map(employeeSchedule.map((schedule) => [schedule.id, schedule]))
  );
    ...
    assignEmployee.mutate(
      { teamId, employeeId, workingDays },
      {
        onSuccess: () => {
          ...
          toast.success("Horario del empleado actualizado exitosamente.", {
            id: "update-employee-schedule",
          });
        },
        onError: (error) => {
          toast.error(error.message, { id: "update-employee-schedule" });
        },
      }
    );
  }
  ...
  React.useEffect(() => {
    if (open === false) {
      setDays(new Map(employeeSchedule.map((schedule) => [schedule.id, schedule])));
    }
  }, [open, setDays, employeeSchedule]);
