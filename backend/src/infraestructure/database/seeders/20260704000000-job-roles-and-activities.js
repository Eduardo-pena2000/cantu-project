...
/**
 * Seeder: Job Roles y Actividades base de Carnes Cavi
 * Fuente: Manual de Procesos (08.03.2022) y Responsabilidades por puesto
 *
 * Este seeder carga los 12 roles de trabajo y sus actividades diarias
 * para TODAS las tiendas existentes en la BD.
 *
 * Los IDs de roles empiezan en 2000 y los de actividades en 3000
 * para evitar conflictos con datos existentes.
 */
...
// Actividades por rol con su deadline predeterminado (HH:mm)
const ACTIVITIES_BY_ROLE = {
  enc_tienda: [
...
module.exports = {
  async up(queryInterface, Sequelize) {
    // 1. Obtener todas las tiendas existentes
    ...
    if (stores.length === 0) {
      console.log("⚠️  No hay tiendas en la BD. El seeder no tiene nada que hacer.");
      return;
    }

    // 2. Obtener o crear área genérica por tienda
    // Asumimos que cada tienda puede no tener áreas o tener una área general
    // Usamos la primera área disponible de cada tienda
    const now = new Date();
    let jobRoleId = 2000;
    let activityId = 3000;
    ...
    for (const store of stores) {
      ...
      // Verificar que no existan ya los roles para esta tienda
      const existingRoles = await queryInterface.sequelize.query(
        `SELECT id FROM job_roles WHERE store_id = ${storeId} AND id >= 2000 AND deleted_at IS NULL`,
        { type: queryInterface.sequelize.QueryTypes.SELECT }
      );

      if (existingRoles.length > 0) {
        console.log(`⏭️  Tienda ${storeId} ya tiene roles base. Saltando...`);
        continue;
      }

      // Obtener la primera área de la tienda (para asociar actividades)
      ...
      if (areas.length === 0) {
        console.log(`⚠️  Tienda ${storeId} no tiene áreas. Creando área general...`);
        ...
        const [newArea] = await queryInterface.sequelize.query(
      ...
      console.log(`✅ Creando roles y actividades para tienda ${storeId}...`);
      ...
      for (const role of JOB_ROLES) {
        ...
        if (activities.length > 0) {
          const activityRows = activities.map((act) => ({
            ...
            default_deadline: act.default_deadline,
            area_id: areaId,
    ...
    console.log("✅ Seeder completado: roles y actividades base cargadas.");
  },

  async down(queryInterface, Sequelize) {
    // Eliminar actividades y roles creados por este seeder
    await queryInterface.sequelize.query(
      "DELETE FROM activities WHERE id >= 3000",
    );
    await queryInterface.sequelize.query(
      "DELETE FROM job_roles WHERE id >= 2000",
    );
    console.log("↩️  Seeder revertido.");
  },
};
